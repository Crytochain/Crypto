...
/**
 * Determines if the signature is valid from LBR sign
...
/**
 * Add the prefix to the message as identification from LBR system
 * if the signature is valid from LBR sign
 ...
 * 1-24: 'LBRNode Signed Message:' : actual header string
 * 25: 0x10 = '\n' : newline ASCII
...
var addPrefixToMessage = function addPrefixToMessage(message) {
  //Header: \x19LBRNode Signed Message:\n
 ...
 * then use `ecsign` to produce the same signature as the `mc_sign` from LBR nodes.
 * call for a given `message`, or fed to `ecrecover` along with a signature to recover the public key
 * For non-HEX string, to comply with LBR node sign process.
...
var signMcMessage = function signMcMessage(message, privateKey) {
  ...
  // add LBR sign header as LBR vnode before the 
  // "\x19LBRNode Signed Message:\n"
  var prefixMsg = addPrefixToMessage(toBuffer(message));
...
var signTransaction = function (tx, privateKey) {
  ...
  try {
    ...
    // for LBR, keep 9 fields instead of 6
    var vPos = 9;
