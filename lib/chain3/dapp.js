 ...
 * This requires both VNODE and SCS monitor are connected.
 * on the MicroChain.
 *
...
var checkForMicroChainDapp = function(dapp, callback){
    ...
    if ( dapp._scs.getTransactionReceipt(dapp.transactionHash) != 1 ){

        if (callback)
            callback(new Error('Failed to create Dapp on the MicroChain!'));
        else
            throw new Error('Failed to create Dapp on the MicroChain!');
    }
