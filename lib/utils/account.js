...
function verifyMcSignature(message, signedData, srcAddr) {
  ...
  if (utils.isHexString(message)){
    ...
    // For non-HEX string, encode it to HEX string using RLP encode method
...
function signMcMessage(message, privateKey) {
  ...
  if (utils.isHexString(message)){
     prefixMsg = addPrefixToMessage(toBuffer(message));
  ...
  console.log("R HEX:", bufferToHex(newsign.r));//, bufferToHex(newsign.s))
  console.log("R HEX:", makeEven(bufferToHex(newsign.r)));

  console.log("S HEX:", bufferToHex(newsign.s));
  console.log("S HEX:", makeEven(bufferToHex(newsign.s)));
  var v = makeEven(bufferToHex(newsign.v));
  var r = makeEven(bufferToHex(newsign.r));
  var s = makeEven(bufferToHex(newsign.s));
