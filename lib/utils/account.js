...
function ecsign(msgHash, privateKeyStr) {
  ...
  if (isHexString(msgHash)) {
    ...
    throw new Error('Invalid HASH input! Not a HEX string');
  }
 ...
 * message    -  Original message, will be encoded to RLP to verify
...
function verifyMcSignature(message, signedData, srcAddr) {
  ...
  // Add MOAC prefix to the message
  // Encode the data to avoid format misrepresentation
  if (utils.isHexString(message)){
    console.log("No need for RLP")
    var prefixMsg = addPrefixToMessage(message);

  }else{
    var prefixMsg = addPrefixToMessage(RLP.encode(message));
  }
  ...
  try {
    ...
    console.log("recover:",recoverAddr.toUpperCase());
    console.log("src:", srcAddr.toUpperCase());
 ...
 * 27: 0x74 = 116 = 't' : actual input byte array;
 * @param {Buffer} 
 * @return {Buffer} 
 */
 ...
 * The output of this function can be fed into `ecsign` to produce the same signature as the `mc_sign`
...
/**
 * Returns the keccak-256 hash of `message`, notice this message should be encoded with RLP encoding
 * to prevent misrepresentation.
 * prefixed with the header used by the `mc_sign` RPC call, 
...
function signMcMessage(message, privateKey) {
  ...
  //Convert the input String to HEX format
  // with RLP encoding 
  if (utils.isHexString(message)){
    console.log("No RLP", message);
    var prefixMsg = addPrefixToMessage(message);
  }else{
    var prefixMsg = addPrefixToMessage(RLP.encode(message));
  }
...
console.log("prefixMsg:", prefixMsg)
  var inHash = Hash.keccak256(prefixMsg);//
console.log("signHash", inHash);
console.log("Buff:", toBuffer(inHash));

  var newsign = ecsign(inHash, stripHexPrefix(privateKey));
  var  v = trimLeadingZero(makeEven(bufferToHex(newsign.v)));
  var  r = padWithZeroes(bufferToHex(newsign.r), 64);
  var  s = padWithZeroes(bufferToHex(newsign.s), 64);
console.log("V:",v)
console.log("R:",r)
console.log("S:",s)
