...
var addDappFunctions = function (mcdapp) {
    ...
    mcdapp.abi.filter(function (json) {    
        ...
        return new MicroFunction(mcdapp._mc, mcdapp._scs, json, mcdapp.address, mcdapp.via);
    }).forEach(function (f) {
 ...
 * @param {Array} abi
 */
var MicroChain = function (mc, scs, inName) {
    this._mc = mc;
    this._scs = scs;
    this.mcType = inName;
    this.via = null;
 ...
 * Microchain has Dapp deployed by using getDappState.
 * 
 * @method getDappBase
 * @param {Address} MicroChain address (required)
 * @param {Function} callback {optional)
 ...
 * 
 */
MicroChain.prototype.getDappBase = function (inAddress, callback) {
...
// console.log("MicroChain.prototype.at:", this.mcType);
  ...
  dappAddrList =  this._scs.getDappAddrList(inAddress);
  ...
  if (dappAddrList.length > 0){
    ...
    var mcBase = new DappBase.DappBase(this._mc, this._scs, this.baseabi, inAddress, this.via);
...
MicroChain.prototype.getDapp = function (inAddress, inAbi, callback) {
