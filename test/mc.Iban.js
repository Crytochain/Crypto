    ...
    describe('Iban', function () {
        tests.forEach(function (test) {
            it('toAddress() should transform iban to address: ' +  test.address, function () {
                if(test.error) {
                    assert.throws(mc.Iban.toAddress.bind(mc.Iban, test.direct));
                } else {
                    assert.deepEqual(mc.Iban.toAddress(test.direct), test.address);
                }
            });
            it('toIban() should transform address to iban: ' +  test.address, function () {
                if(test.error) {
                    assert.throws(mc.Iban.toIban.bind(mc, test.address));
                } else {
                    assert.deepEqual(mc.Iban.toIban(test.address), test.direct);
                }
            });
        });
    });
